...
> 版本：v1.0.0 | 日期：2026-08-02 | 状态：生产加固（SSE 断线重连、首包延迟优化、Embedding 连接池、多库推测搜索、Prompt 瘦身、Prompt Injection 防御、PII 脱敏、审计哈希链、Worker 健康分级、JWT 启动校验、SQLite 并发修复、评估 Webhook 告警）
...
13. [企业级运营增强（v0.0.9）](#13-企业级运营增强v009)
14. [生产加固（v1.0.0）](#14-生产加固v100)
15. [附录：配置速查](#15-附录配置速查)
16. [版本历史](#16-版本历史)
...
> **v0.0.9 新增**。本版本聚焦生产运营层面的八大增强，覆盖进程架构、成本管控、质量闭环、文档治理和差异化能力。

### 13.1 Worker 进程分离与分布式调度锁

#### 问题

v0.0.8 及之前，12 个后台 Worker（摄取、评测、对账、修复等）全部在 API 进程的 `lifespan` 中启动。多副本部署时存在重复执行风险，且 API 与 Worker 无法独立扩缩容。

#### 设计

**Worker 注册表**（`app/worker/registry.py`）：声明式定义所有 12 个 Worker 的模块路径、类名和描述。
...
| 配置 | 默认值 | 说明 |
|------|--------|------|
| `WORKER_MODE` | `embedded` | `embedded`：Worker 随 API 进程启动（向后兼容）；`standalone`：API 进程不启动任何 Worker |

**分布式锁**（`app/worker/distributed_lock.py`）：
- PostgreSQL 后端：`pg_try_advisory_lock` 实现 at-most-one 语义
- 本地后端：内存 dict + TTL（开发/测试用）
- 支持 acquire / release / renew 三个操作，TTL 过期自动释放

**向后兼容**：默认 `WORKER_MODE=embedded`，行为与 v0.0.8 完全一致。

---

### 13.2 语义缓存与 Token 成本控制

#### 语义缓存（`app/rag/semantic_cache.py`）

高频相似问题命中缓存直接返回，跳过 LLM 调用：

- **匹配算法**：查询 embedding 与缓存 embedding 的 cosine 相似度 ≥ 阈值（默认 0.96）
- **作用域**：按 `tenant_id + kb_scope` 隔离，不同租户/知识库之间缓存不互通
- **TTL**：默认 3600 秒自动过期
- **失效**：知识库索引 generation 切换时调用 `invalidate_kb` 清除该 KB 全部缓存
- **后端**：memory（测试）/ Redis（生产）

| 配置 | 默认值 | 说明 |
|------|--------|------|
| `SEMANTIC_CACHE_ENABLED` | `false` | 全局开关 |
| `SEMANTIC_CACHE_THRESHOLD` | `0.96` | 相似度命中阈值 |
| `SEMANTIC_CACHE_TTL_SEC` | `3600` | 缓存有效期 |

#### Token 配额（`app/rag/quota.py`）

每租户每日 Token 预算硬限制：

- 超限返回 HTTP 429 + 剩余重置时间
- Redis 原子 INCR 计数，每日自动过期重置
- 支持按租户自定义配额（查询 `t_token_quota` 表）

| 配置 | 默认值 | 说明 |
|------|--------|------|
| `TOKEN_QUOTA_ENABLED` | `false` | 全局开关 |
| `TOKEN_QUOTA_DAILY_DEFAULT` | `1000000` | 默认每租户每日 Token 上限 |
...
```
用户反馈(👍👎) → 自动标注(base_case) → 质量告警 → 评测集扩充(golden)
                                                        ↓
零结果查询 → DBSCAN 聚类 → 知识缺口报告 → 知识库补充
                                                        ↓
评测 Run 历史 → 指标分析 → 超参调优建议
```

#### 反馈驱动告警（`app/evaluation/quality_alert.py`）

- 同一 KB 连续 3 个负反馈触发 `quality_alert`
- 正反馈 ≥ 3 且无负反馈的 case 满足自动提升为 golden 的条件

#### 零结果查询聚类（`app/evaluation/query_clusters.py`）

- 数据源：`t_rag_trace_run` 中 `final_count=0` 或有 `rejection_reason` 的记录
- 算法：基于 cosine 距离的 DBSCAN（eps=0.3, min_samples=3）
- 输出：每个 cluster 的代表问题、频次、涉及 KB

#### 超参调优建议（`app/evaluation/tuning.py`）

| 指标异常 | 建议 |
|----------|------|
| avg Recall@K < 0.6 | 增大 `RETRIEVAL_PER_CHANNEL_TOP_K`（12→20） |
| 拒答率 > 30% | 降低 `RETRIEVAL_RRF_MIN_SCORE`（0.012→0.008） |
| P95 延迟 > 5000ms | 降低 `RETRIEVAL_MAX_CANDIDATES`（40→25） |
...
- `APPROVAL_ENABLED=false`（默认关闭）时，上传直接进入 QUEUED（向后兼容）
- 审批人：KB 的 WRITE/ADMIN 权限持有者、KB 创建者、admin 角色
- 核心模块：`app/services/approval.py`（ApprovalStateMachine + 权限校验）

| 配置 | 默认值 | 说明 |
|------|--------|------|
| `APPROVAL_ENABLED` | `false` | 文档审批全局开关 |

---

### 13.5 表格精确问答（Table QA）

对 `block_type=table` 的结构化表格做精确单元格级回答（`app/rag/table_qa.py`）：

**查询识别**：包含"多少"、"最大/最小"、"平均"、"第几行"、列名引用等模式。

**支持操作**：

| 操作 | 触发词示例 | 说明 |
|------|-----------|------|
| lookup | "市场部有多少人" | 定位行+列，返回单元格值 |
| sum | "总销售额" | 数值列求和 |
| max / min | "GDP最大值" | 数值列极值 |
| avg | "平均成绩" | 数值列均值 |
| count | "一共多少条记录" | 行数统计 |

**列匹配策略**：数量意图词（人数/金额）优先匹配对应列头，避免"部门"列被误选为数值列。
...
后端模块 `app/rag/graph/expansion.py` 提供：

- **`expand_neighbors(graph, node_id, depth, limit, exclude_ids)`**：BFS 从指定节点展开 N 跳邻居，支持排除已展示节点、限制返回数量
- **`mark_expandable(graph, visible_ids)`**：标注哪些节点有未展示的邻居（前端显示"+"按钮）
- **`merge_graph_views(existing, incoming)`**：增量合并新子图到已有画布，自动去重

前端交互（规划中）：节点双击展开邻居、右键菜单（展开/聚焦/隐藏）、展开历史栈（支持收回）。

---

### 13.7 文档版本 Diff

同一文档重上传后，展示新旧版本的差异（`app/services/document_diff.py`）：

- **版本记录**：每次重处理/重上传创建新 version（递增版本号）
- **Diff 计算**：基于 `content_hash` 集合差异 → added / removed / modified
- **输出**：`DiffSummary(added=[...], removed=[...], modified=[...])`

---

### 13.8 对话分析仪表盘

纯计算层 `app/services/analytics.py`，从预取数据中聚合运营指标：
...
| 平均轮次 | user 消息数 / 会话数 |
| 满意度趋势 | 按日期聚合正/负反馈 |
| 热门话题 Top 10 | trace query 关键词提取 + 频次排序 |
| 峰值时段 | 按小时聚合 user 消息 |
| 拒答率 | refusal 的 trace / 总 trace |

前端页面（规划中）：`/admin/analytics` 路由，折线图 + 柱状图 + 数字卡片。
...
SEMANTIC_CACHE_TTL_SEC=3600

# === Token 配额 ===
TOKEN_QUOTA_ENABLED=false
TOKEN_QUOTA_DAILY_DEFAULT=1000000
...
> **v1.0.0 新增**。本版本完成优化方案全部 P1 优先级条目（10 项），覆盖性能、安全、可靠性和运营四个维度。

### 14.1 SSE 流式体验增强（1.1 / 2.1）

#### 首包延迟优化

`event_stream()` 生成器的第一个 yield 为 `connected` 心跳事件，在任何 DB 操作之前发送，确保客户端立即收到首字节：

```python
yield "event: connected\ndata: {}\n\n"
```

前端 `useStreamResponse.ts` 新增 `onConnected` 回调和 `connected` 事件分发。

#### 多阶段进度

后端在 RAG 管线各子阶段推送 4 个 `progress` 事件：

| 阶段 | stage | 触发时机 |
|------|-------|---------|
| 理解 | `understanding` | pipeline.run() 之前 |
| 检索 | `retrieving` | pipeline.run() 之前（已有，保留） |
| 排序 | `ranking` | pipeline.run() 返回后、prompt 组装前 |
| 生成 | `generating` | LLM 流式输出前（已有，保留） |

#### 前端断线重连

`createStreamResponse` 增加自动重连机制：

- 最多 3 次重试，指数退避（1s → 2s → 4s）
- 仅对网络级错误（fetch 失败、连接中断）重试，HTTP 4xx/5xx 不重试
- 显式 abort 不重试
- 新增 `onReconnect(attempt)` 回调和 `retryCount` getter
...
`app/llm/embedding.py` 引入模块级共享 `httpx.AsyncClient`，替代每次请求创建新客户端：
...
`_call_with_retry` 复用该客户端，通过 `client.post(..., timeout=timeout_sec)` 覆盖单次请求超时。

#### 多库推测搜索

`pipeline.py` 的推测搜索从仅支持单 scope 扩展到全部 speculative scopes：

```python
# 旧：len(speculative_scopes) == 1 才触发
# 新：任意非空 speculative_scopes 均触发
if settings.ttft_speculative_search and speculative_scopes:
    speculative_task = asyncio.create_task(
        self._search_vector_scopes([ctx.question], speculative_scopes, ...)
    )
```

#### 超时降低

`query_understanding_timeout_sec` 默认值从 `20.0` 降至 `8.0`，减少改写/意图识别对首 token 延迟的拖累。

---

### 14.3 Prompt 瘦身（2.4）

历史消息从固定 `history[-16:]` 改为 token 预算截断：

```python
def _truncate_history_by_tokens(history, max_tokens):
    result = []
    total = 0
...
预算由 `conversation_context_max_tokens`（默认 2000）控制，保留最近消息，避免超长上下文浪费 LLM 窗口。

---

### 14.4 Prompt Injection 防御（3.5）

新增模块 `app/security/injection.py`，提供三层防御：

#### 输入检测

`detect_injection(text)` 使用 18 条中英文正则规则检测常见注入模式（忽略指令、DAN 模式、泄露提示词、越狱等），命中时返回 HTTP 400。

#### 证据 sanitize

`sanitize_evidence(text)` 在检索证据注入 prompt 之前：
- 移除包含注入关键词的 HTML 注释（`<!-- 忽略以上指令 -->`）
- 过滤以注入短语开头的行

#### PII 脱敏

`detect_pii(text)` / `mask_pii(text)` 检测并脱敏三类 PII：

| 类型 | 正则 | 脱敏示例 |
|------|------|---------|
| 身份证 | 18 位标准格式 | `110101****1234` |
| 手机号 | `1[3-9]\d{9}` | `138****5678` |
| 邮箱 | 标准邮箱格式 | `te***@example.com` |

重叠匹配去重：同一位置身份证内嵌手机号只保留较长匹配。

全部功能由 `INJECTION_DETECTION_ENABLED=true`（默认开启）控制。
...
每次成功对话后调用 `record_audit`，记录 `biz_type=chat`、问题摘要、conversation_id、kb_id。

#### 哈希链

`BizChangeLog.after_snapshot` 新增两个字段：

| 字段 | 说明 |
|------|------|
| `_prev_hash` | 上一条审计记录的 `_entry_hash`（首条为 `"genesis"`） |
| `_entry_hash` | 本条记录的 SHA-256（基于 id + biz_type + biz_id + operation_type + prev_hash + create_time） |

实现位于 `app/audit/service.py` 的 `_compute_entry_hash` 和 `record_audit` 内部。

#### 独立保留期

`AUDIT_RETENTION_DAYS`（默认 180）控制审计日志保留期，独立于 trace 保留期（30 天）。`retention.py` 中新增对 `BizChangeLog` 的定期清理。

---

### 14.6 Worker 健康分级（4.4）

#### 关键 Worker 定义

```python
_CRITICAL_WORKERS = {"ingestion_job_worker", "index_reconciliation_worker", "index_repair_worker"}
```

关键 Worker 启动失败时：
1. 记录到 `_critical_worker_failures` 列表
2. 调用 `record_system_error` 上报

#### Readiness 暴露

`health.py` 的 `readiness_checks()` 新增：

```python
checks["workers"] = "error:ingestion_job_worker"  # 或 "ok"
```
...
- `lifespan()` 中检测 `jwt_secret_key == "change-me-in-production"` 时输出 CRITICAL 日志
- `readiness_checks()` 新增 `jwt` 检查项：默认密钥时返回 `error:default_secret`
- `.env.example` 中 `JWT_SECRET_KEY` 改为 `<GENERATE_A_RANDOM_SECRET>` 占位符

---

### 14.8 SQLite 并发修复（4.2）

`event_stream()` 内所有写操作改用独立短 session，避免与请求级 session 竞争写锁：

| 操作 | 原实现 | 新实现 |
|------|--------|--------|
| save_message | 请求级 `db` | `async_session_factory()` 独立 session |
| maybe_summarize | 请求级 `db` | 独立 session |
| trace.finalize | 请求级 `db` | 独立 session |
| 错误路径 finalize | 请求级 `db` + rollback | 独立 session |
...
`app/evaluation/quality_alert.py` 新增 `send_webhook_alert`，支持 DingTalk / Feishu / Slack 格式：

```python
await send_webhook_alert(
    webhook_url,
    title="质量告警",
    metrics={"recall@5": 0.65},
    failed_gates=[{"metric": "recall@5", "value": 0.65, "operator": ">=", "threshold": 0.75}],
)
```

配置：`EVALUATION_WEBHOOK_URL`（默认空，不发送）。

#### CI 模板

`.github/workflows/evaluate.yml` 提供评测工作流模板，在 RAG/LLM/配置变更时触发。
...
EVALUATION_ALERT_MIN_QUALITY_SCORE=0.7
```
...
| 版本 | 日期 | 主题 | 详细章节 |
|---|---|---|---|
| v1.0.0 | 2026-08-02 | 生产加固：SSE 断线重连 + 首包延迟优化 + Embedding 连接池 + 多库推测搜索 + Prompt 瘦身 + Prompt Injection 防御 + PII 脱敏 + 审计哈希链 + Worker 健康分级 + JWT 启动校验 + SQLite 并发修复 + 评估 Webhook 告警 | [14. 生产加固](#14-生产加固v100) |
| v0.0.9 | 2026-08-02 | 企业级运营增强：Worker 进程分离 + 语义缓存 + Token 配额 + 质量闭环 + 文档审批 + 表格 QA + 图谱交互展开 + 版本 Diff + 对话分析 | [13. 企业级运营增强](#13-企业级运营增强v009) |
| v0.0.8 | 2026-07-28 | 跨库公平召回：KB Interleave + Per-KB 配额 + Fallback Pool + 来源归因 + Rewrite/Intent 降级 | [3. 召回逻辑](#3-召回逻辑) |
| v0.0.7 | 2026-07-27 | 可信语义知识图谱：三层混合图谱 + 原文证据校验 + 增量维护 + 原地回填 | [7. Graph RAG](#7-graph-rag) |
| v0.0.6 | 2026-07-25 | 跨知识库 Graph RAG：全库检索 + 跨库实体桥 + 强制 Graph + 分类型知识星图 | [3. 召回逻辑](#3-召回逻辑)、[7. Graph RAG](#7-graph-rag) |
| v0.0.5 | 2026-07-20 | 企业级 RAG 数据面加固：非破坏索引 + 一致性摄取 + 持久化任务 + 企业评测 | [2. 分块逻辑](#2-分块逻辑)、[3. 召回逻辑](#3-召回逻辑)、[5. 评测体系](#5-评测体系)、[9. 批量导入](#9-批量导入与文档去重)、[10. 异步摄取](#10-异步摄取与增量索引) |

---

> **文档维护**：本文档基于 `flavorag-server/app/` 与 `flavorag-frontend/src/` 的实际源码编写，与代码保持同步更新。各版本详细设计见对应 SDD 文档。
