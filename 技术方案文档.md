...
> 版本：v0.0.6 | 日期：2026-07-28 | 状态：跨知识库 Graph RAG（203/203 后端测试、6/6 前端测试、生产构建、200 节点视觉 QA 通过）
...
| 跨库关系 | 实体持久化 `tenant_id + normalized_name`；同租户不同库的同名实体写入/读取为 `CROSS_KB_RELATED` |
| 图谱容量 | 公共图接口默认且最多 200 节点；查询 `limit + 1`，仅在实际超过上限时返回 `truncated=true` |
| 知识星图 | 按知识库聚类、来源图例、跨库虚线桥、画布拖拽、指针锚定滚轮缩放、按钮缩放/复位 |
...
| 验证 | 后端 203 项全量测试、前端 6 项单测、TypeScript、Vite 生产构建、200 节点桌面/390px 窄屏 QA；真实 API 完成知识库创建与清理烟测 |
...
- 同租户不同 KB 的规范化同名实体使用 `CROSS_KB_RELATED` 连接；
...
v0.0.6 增加第二类确定性关系：**跨库同名实体桥**。实体名先做 Unicode `casefold`，再去除空格、标点和下划线得到 `normalized_name`。只有 `tenant_id` 相同、`kb_id` 不同、`normalized_name` 非空且相同的节点才会连接：
...
WHERE other.kb_id <> row.kb_id AND other.id > entity.id
MERGE (entity)-[:CROSS_KB_RELATED {cross_kb: true}]->(other)
```

这是可解释、幂等且不依赖 LLM 的最低可靠关联。历史节点尚未写入该边时，组合图读取还会对可见节点执行同租户范围内的同名匹配，生成等价的只读跨库桥。
...
#### 7.2.4 LightRAG 层：构造文档文本 → HTTP 调用
...
公共接口把 `limit` 限制在 1..200。内部读取 `limit + 1` 个节点：正好 200 个时完整返回且 `truncated=false`，第 201 个存在时截取关联度最高的 200 个并返回 `truncated=true`。节点附带 `knowledgeBaseId/knowledgeBaseName`，边附带 `type/crossKnowledgeBase`。

LightRAG 同样有 `/graphs` 接口（`fetch_graph`），返回格式与 Neo4j 一致，前端可以切换数据源。

前端 `KnowledgeGraphPanel` 使用确定性知识库聚类布局，而不是每次随机抖动的力导向图。交互状态为 `{x, y, scale}`：拖动 SVG 画布改变平移量；滚轮缩放以鼠标位置为锚点并限制在 45%..250%；按钮提供缩放和复位。局部关系使用实线，跨库桥使用紫色虚线。Graph 通道实际返回证据后，前端从查询命中的实体开始选择一条确定性连通路径，播放边光流和节点呼吸；系统设置 reduced motion 时只保留静态高亮。
...
> **文档维护**：本文档基于 `flavorag-server/app/` 与 `flavorag-frontend/src/` 的实际源码编写，与代码保持同步更新。各版本特性：v0.0.2 批量导入/异步摄取/增量索引，v0.0.3 HyDE 假设文档嵌入，v0.0.4 Mem0 长期记忆与用户画像，v0.0.5 非破坏索引 generation、一致性摄取、持久化任务、企业评测、共享源存储与生产运维加固，v0.0.6 权限安全的跨知识库检索、跨库实体桥、强制 Graph RAG 与 200 节点知识星图。
