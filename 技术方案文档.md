...
> 版本：v0.0.2 | 日期：2026-07-27 | 状态：生产就绪（144/144 测试全绿）
...
8. [批量导入与文档去重](#8-批量导入与文档去重)
9. [异步摄取与增量索引](#9-异步摄取与增量索引)
10. [系统监控与健康检查](#10-系统监控与健康检查)
11. [附录：配置速查](#11-附录配置速查)
...
一句话概括：**用户提问 → 问题改写 → 意图识别 → 多路并行检索 → RRF 融合 → 去重 → 近邻补偿 → Rerank 重排序 → 大模型生成回答 → 返回带来源引用（含图片/表格）的答案。**
...
### 5.1 Prometheus 指标（v0.0.2 升级至 15 个指标）
