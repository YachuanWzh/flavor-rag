...
Flavor-RAG 的可观测性体系有三层，各司其职：

- **Prometheus + Grafana（指标层）**：实时数值监控——QPS、延迟、错误率、队列深度，看大盘、设告警
- **OpenTelemetry + Jaeger（链路层）**：分布式追踪——一次请求经过了哪些步骤、每步花了多少毫秒、瓶颈在哪
- **TraceLogger + PostgreSQL（业务审计层）**：把追踪数据持久化到数据库——管理后台可以查历史记录、看输入输出快照

```mermaid
flowchart TB
    subgraph 应用层["FlavorAG Server"]
        direction LR
        MW["MetricsMiddleware<br/>HTTP 请求拦截"]
        OTL["OpenTelemetry<br/>链路追踪"]
        TL["TraceLogger<br/>业务追踪写PG"]
    end

    subgraph 采集层["数据收集"]
        PM["Prometheus<br/>每15秒拉取 /metrics"]
        JE["Jaeger<br/>接收 OTLP 追踪数据"]
    end

    subgraph 展示层["可视化"]
        GF["Grafana<br/>实时大盘 + 告警"]
        JU["Jaeger UI<br/>火焰图 + 耗时分析"]
        TP["TracesPage<br/>管理后台历史查询"]
    end

    MW -->|"HTTP QPS / 延迟"| PM --> GF
    TL -->|"RAG运行 / 检索耗时"| PM
    OTL -->|"OTLP span 数据"| JE --> JU
    TL -->|"写入 t_rag_trace_run / _node"| TP
```

> **一句话**：Prometheus + Grafana 看"现在怎么样"，Jaeger 看"为什么这么慢"，TraceLogger 看"刚才那一次发生了什么"。

### 5.1 Prometheus 指标（实时数值监控）

#### 5.1.1 整体架构

```
FlavorAG Server  :9090/metrics
        ↑ (Prometheus 每 15s 抓取一次)
    Prometheus  :9091
        ↑ (Grafana 读取 Prometheus 数据)
    Grafana     :3000  (预置大盘)
```
...
# Jaeger UI   → http://localhost:16686 (需要 OTEL_ENABLED=true)
```

#### 5.1.2 指标体系一览

所有指标前缀 `flavorag_`，由 `app/observability/metrics.py` 定义。系统采用低基数（low-cardinality）设计——标签值来自有限集合（路由模板、通道名、模型名），避免数据爆炸。

| 指标名 | 类型 | 用途 | 来源 |
|--------|------|------|------|
| `flavorag_http_requests_total` | Counter | HTTP 请求计数（按 method + path + status） | MetricsMiddleware |
| `flavorag_http_request_duration_seconds` | Histogram | HTTP 请求延迟分布（P50/P95/P99） | MetricsMiddleware |
| `flavorag_rag_runs_total` | Counter | RAG 流水线完成次数（按 status） | TraceLogger.finalize() |
| `flavorag_retrieval_channel_total` | Counter | 各检索通道调用次数（按 channel + status） | governance.py |
| `flavorag_retrieval_channel_duration_seconds` | Histogram | 各检索通道耗时分布 | governance.py |
| `flavorag_rerank_duration_seconds` | Histogram | Rerank 阶段耗时 | reranker.py |
| `flavorag_rerank_fallback_total` | Counter | Rerank 降级次数 | reranker.py |
| `flavorag_llm_first_token_seconds` | Histogram | LLM 首 Token 延迟（按模型） | llm/client.py |
| `flavorag_llm_stream_duration_seconds` | Histogram | LLM 完整流式输出耗时 | llm/client.py |
| `flavorag_llm_stream_failures_total` | Counter | LLM 调用失败次数 | llm/client.py |
| `flavorag_circuit_breaker_open` | Gauge | 熔断器状态（1=打开 0=关闭） | governance.py |
| `flavorag_ingestion_jobs_total` | Counter | 入库任务完成次数（按 result） | ingestion_jobs.py |
| `flavorag_ingestion_job_duration_seconds` | Histogram | 入库任务耗时 | ingestion_jobs.py |
| `flavorag_ingestion_queue_depth` | Gauge | 入库队列积压情况（按 status） | ingestion_jobs.py |

#### 5.1.3 Grafana 预置大盘

大盘文件位于 `docker/observability/grafana/dashboards/flavorag-overview.json`，30s 自动刷新，包含 8 个面板：

| 面板 | 类型 | 内容 |
|------|------|------|
| HTTP QPS by route | 时序图 | 每个 API 路由的每秒请求数 |
| HTTP p95 latency | 时序图 | 每个 API 路由的 P95 响应延迟 |
| RAG runs by status | 时序图 | success / error / rejected 分类统计 |
| Retrieval channel p95 latency | 时序图 | vector / keyword / graph 三通道延迟对比 |
| LLM time-to-first-token p95 | 时序图 | 按模型的首 Token 响应速度 |
| Ingestion queue depth | 时序图 | 入库队列 pending / running / failed 状态 |
| Ingestion jobs finished / min | 时序图 | 每分钟完成（succeeded/failed）的入库任务 |
| Open circuit breakers | 数字面板 | 当前有多少个熔断器处于打开状态 |
...
指标采集完全无侵入——`MetricsMiddleware` 自动拦截所有 HTTP 请求，业务代码只需在关键路径上调用 `metrics.labels().inc()` 或 `.observe()`。

### 5.2 OpenTelemetry 链路追踪（分布式追踪）

#### 5.2.1 核心思路

**把现有的 TraceLogger 记录自动镜像一份到 Jaeger**，让你用火焰图定位瓶颈，同时不影响已有数据库追踪。

Flavor-RAG 的业务追踪（TraceLogger）已经记录了每个 RAG 步骤的精确 `start_time` / `end_time`。OTEL 模块做的事是：把同样的时间数据以 OTLP 标准协议发送给 Jaeger，形成标准的 Trace → Span 结构。

```mermaid
sequenceDiagram
    participant App as FlavorAG Server
    participant TL as TraceLogger
    participant DB as PostgreSQL
    participant OTEL as OTEL Module
    participant JGR as Jaeger

    App->>TL: trace_query("什么是RAG?")
    TL->>DB: INSERT t_rag_trace_run
    TL->>OTEL: start_rag_span("rag.run")
    OTEL->>JGR: 创建 Root Span

    App->>TL: trace_node("rewrite", ...)
    TL->>DB: INSERT t_rag_trace_node
    TL->>OTEL: record_child_span(parent, "rag.rewrite", t1, t2)
    OTEL->>JGR: 添加子 Span (含精确时间戳)

    App->>TL: trace_node("vector_search", ...)
    TL->>DB: INSERT t_rag_trace_node
    TL->>OTEL: record_child_span(...)
    OTEL->>JGR: 添加子 Span

    App->>TL: finalize(...)
    TL->>OTEL: end_rag_span(span, attributes)
    OTEL->>JGR: 结束 Root Span + 属性上报
```

#### 5.2.2 设计要点

1. **完全可选，默认关闭**：`OTEL_ENABLED=true` 才初始化，否则所有函数都是 no-op（零开销）
2. **失败不影响主流程**：`opentelemetry-sdk` 包未安装时打印 warning 降级，不会 crash
3. **Span 是"事后补录"**：TraceLogger 已记录精确的 `start_time` / `end_time`，OTEL 直接用 `start_span(start_time=...)` + `span.end(end_time=...)` 创建回溯 Span——无论数据库写入多慢，Span 的时间戳始终精确
4. **FastAPI 自动打点**：`FastAPIInstrumentor` 自动为每个 HTTP 请求创建 Span，可追踪中间件层耗时

#### 5.2.3 Span 结构

一次 RAG 问答在 Jaeger 中的 Trace 树：

```
rag.run (root)
├── rag.rewrite.normalize          # Layer 1: 术语映射
├── rag.rewrite.llm_rewrite        # Layer 3: LLM 改写
├── rag.intent.intent_classify     # 意图识别
├── rag.search.vector_search       # 向量检索 (Milvus)
├── rag.search.keyword_search      # 关键词检索 (ES)
├── rag.search.graph_search        # 图谱检索 (Neo4j + LightRAG)
├── rag.postprocess.fusion         # RRF 融合
├── rag.postprocess.rerank         # Rerank 精排
└── rag.llm.answer                 # LLM 回答生成
```

每个 Span 携带属性：`rag.trace_id`、`rag.tenant_id`、`rag.kb_id`、`rag.node_status`、`rag.recall_count`、`rag.final_count`、`rag.model`、`rag.status`。
...
# 安装 OTEL 依赖
pip install flavorag-server[otel]

# 启动 Jaeger（含在 observability-stack 中）
docker compose -f docker/observability-stack.compose.yaml up -d
# Jaeger UI → http://localhost:16686
```

在 Jaeger UI 搜索 `flavorag-server`，即可看到所有 RAG 请求的火焰图和耗时瀑布，快速定位是检索慢、Rerank 慢还是 LLM 慢。

### 5.3 业务追踪（TraceLogger → PostgreSQL）

#### 5.3.1 总体设计

每次问答生成一个 **Trace Run**，每个步骤（改写 → 意图 → 检索 → Rerank → 回答）拆解为独立的 **Trace Node**，记录耗时、输入输出和状态。所有数据写入 PostgreSQL 持久化。

```mermaid
flowchart LR
    A[Trace Run] --> B[rewrite 改写节点]
    A --> C[intent 意图节点]
    A --> D[search 检索节点]
    A --> E[rerank 重排序节点]
    A --> F[llm_answer 回答节点]
...
| `llm_duration_ms` | LLM 回答耗时 |
| `total_duration_ms` | 全链路总耗时 |
| `recall_count` | 检索召回数 |
| `final_count` | 最终给 LLM 的块数 |
| `model_name` | 使用的模型 |
| `status` | success / error / rejected |
| `rejection_reason` | 拒绝原因 |

**Trace Node**（`t_rag_trace_node`）：

| 字段 | 说明 |
|------|------|
| `trace_run_id` | 关联的 Trace Run |
| `node_type` | 节点类型（rewrite / intent / search / rerank / llm） |
| `node_name` | 节点名称（如 `vector_search`、`keyword_search`） |
| `start_time` / `end_time` | 开始/结束时间 |
| `duration_ms` | 耗时 |
| `input_data` / `output_data` | 输入输出快照（JSON） |
...
- `otel_initialized` / `otel_dependencies_missing`（OTEL 状态）

### 5.6 管理后台可视化

系统提供 `TracesPage`（前端 `flavorag-frontend/src/pages/admin/TracesPage.tsx`）、`HealthPage` 和 `MonitoringPage` 用于：
- 按时间范围查询 Trace 记录
- 查看每次问答的完整链路节点时序
- 对比各环节耗时分布
- 排查异常节点和错误信息
...
| `docker/observability-stack.compose.yaml` | Prometheus + Grafana + Jaeger (可选) | 9091, 3000, 16686 |
